...
class ImitationWrapper(LocoEnv):
    ...
    def _init_buffers(self):
        ...
        # 参考平均速度 (speed_match 用); motion 数组根速度列
        try:
            self.motion_speed_mps = float(np.mean(np.abs(self.motion[:, 55]))) if self.motion.shape[1] > 55 else 1.55
        except Exception:
            self.motion_speed_mps = 1.55
        self.cfg_reward_speed_match = self.reward_cfg.get("speed_match", False)
    ...
    def _effective_motion_steps_from_episode_steps(self, env_idx, episode_steps):
        ...
        speed_match: 参考相位增量按 cmd/ref_speed 缩放 (速度匹配奖励, E2).
        命令速度高 -> 参考播放快 (时间拉伸), 命令低 -> 播放慢.
        """
        steps = (episode_steps - self.warmup_steps).clamp(min=0)
        if getattr(self, "cfg_reward_speed_match", False) and self.motion_speed_mps > 0:
            scale = (self.commands[env_idx, 0].clamp(0.3, 2.5) / self.motion_speed_mps).unsqueeze(-1)
            steps = (steps.float() * scale).long()
        return steps + self.motion_start_offset_buf[env_idx]
    ...
    def step(self, actions):
        ...
        # P0-6: 速度匹配相位累积 (warmup 后每步 += cmd/ref_speed; 连续积分无跳变)
        if getattr(self, "cfg_reward_speed_match", False) and self.motion_speed_mps > 0:
            scale = self.commands[:, 0].clamp(0.3, 2.5) / self.motion_speed_mps
            self.sm_phase_acc += scale.long()  # 累积相位步数
            # warmup 阶段不计入 (同步 standing 参考)
            self.sm_phase_acc = torch.where(
                self.episode_length_buf > self.warmup_steps, self.sm_phase_acc, torch.zeros_like(self.sm_phase_acc)
            )
