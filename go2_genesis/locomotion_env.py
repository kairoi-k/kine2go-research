...
class LocoEnv:
    def __init__(
        ...
        self.cfg_reward_tracking_sigma = reward_cfg.get("tracking_sigma", 0.25)
    ...
    def _init_command_buffers(self):
        if self.command_type is not None:
            ...
            self.prev_commands = torch.zeros_like(self.commands)  # Sol P1: 重采样前的命令
    ...
    def post_physics_step(self):
        ...
        # Sol P1: 保存重采样前的命令 (style/假样本标注用产生 transition 的命令)
    ...
    def _render_headless(self):
        if self._recording:
            ...
            # 侧视角: 相机在狗侧面(运动方向垂直侧), 跟随保持侧视
            # 狗朝+x跑 -> 相机在 y 方向侧面偏前(略斜前方), 稍高
            heading = self.base_quat_to_yaw() if hasattr(self, "base_quat_to_yaw") else 0.0
            side = np.array([-np.sin(heading), np.cos(heading), 0.0])
            fwd = np.array([np.cos(heading), np.sin(heading), 0.0])
            cam_pos = robot_pos + 2.2 * side + 0.4 * fwd + np.array([0, 0, 0.45])
            cam_look = robot_pos + np.array([0, 0, 0.2])
            self._floating_camera.set_pose(pos=cam_pos, lookat=cam_look)
            frame, _, _, _ = self._floating_camera.render()
            if hasattr(self, "_frame_dir") and self._frame_dir is not None:
                Image.fromarray(frame.astype(np.uint8)).save(f"{self._frame_dir}/f{self._frame_count:06d}.png")
                self._frame_count += 1
            else:
    ...
    def start_recording(self, *, record_internal=True):
        ...
        self._recorded_frames = []
        self._frame_dir = _tf.mkdtemp(prefix="gs_rec_")
        self._frame_count = 0
    ...
    def stop_recording(self, save_path=None):
        ...
        if save_path is not None:
            ...
            if hasattr(self, "_frame_dir") and self._frame_dir is not None and self._frame_count > 0:
                fps = int(1 / self.dt)
                ...
                print(f"saved {self._frame_count} frames -> {save_path}")
            else:
